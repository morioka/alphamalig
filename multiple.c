...
multiple_alignment (float **matrix, int output_format)
{
    ...
    if (mem_ok == 1)
    {
        while (num_clusters > 1)
        {
            ...
            if ((clusters[i]->len_seqs >= MAXLENSEQ)
                    || (clusters[j]->len_seqs >= MAXLENSEQ))
            {
            ...
            alignment_clusters (matrix, i, j, info1, info2, line_orig,
                                line_dest, result_cluster1, result_cluster2,
                                &clusters[i]->score);
...
void
create_cluster_files (int i, int j)
{
    int k = 0;
    ...
    {
        pos_seq_clusters_file[seqs[i][k]] =
            add_sequence_cluster (cluster_file_1, seqs[i][k]);
        k++;
    }
    ...
    {
        pos_seq_clusters_file[seqs[j][k]] =
            add_sequence_cluster (cluster_file_2, seqs[j][k]);
        k++;
    }
...
void
find_nearest_clusters (int *i, int *j)
{
...
void
similarity_clusters (float **matrix, long len_cluster1, long len_cluster2,
                     float **info1, float **info2, long *score, int numseqs1,
                     int numseqs2)
{
    int i = 0, j = 0;
    ...
    {
        matrix[i][0] =
            matrix[i - 1][0] + calc_simil_ini (info1[i - 1], numseqs2);
        path_matrix[i][0] = 'a';
    }
    ...
    {
        matrix[0][j] =
            matrix[0][j - 1] + calc_simil_ini (info2[j - 1], numseqs1);
        path_matrix[i][0] = 'e';
    }
    ...
    for (i = 1; i <= len_cluster1; i++)
    {
        ...
        {
            p_diag =
                matrix[i - 1][j - 1] + calc_simil (info1[i - 1], info2[j - 1]);
            p_esq = matrix[i][j - 1] + calc_simil_ini (info2[j - 1], numseqs1);
            p_amunt =
                matrix[i - 1][j] + calc_simil_ini (info1[i - 1], numseqs2);
...
void
show_cars_matrix (char **m, int files, int cols)
{
    int i, j;
...
float
calc_simil_ini (float *freq, int n)
{
...
float
calc_simil (float *freq1, float *freq2)
{
...
/****************************************/
float
intercluster (float *freq1, float *freq2)
{
    float sum;
    int i, j;
    sum = 0;
                    ...
                    char *result_cluster1, char *result_cluster2, long *score)
{
    ...
    similarity_clusters (matrix, clusters[i]->len_seqs, clusters[j]->len_seqs,
                         info1, info2, score, clusters[i]->num_seqs,
                         clusters[j]->num_seqs);
    ...
    align_clusters (clusters[i]->len_seqs, clusters[j]->len_seqs,
                    result_cluster1, result_cluster2, &len);
    rebuild_new_cluster (line_orig, line_dest, result_cluster1, result_cluster2,
                         i, j, len);
...
void
recalculate_matrix (int i, int j)
{
    int aux = 0, n = 0;
    ...
    while (aux < num_seqs)
    {
        if (i != aux)
        {
            ...
            {
                score_matrix[i][aux] =
                    (score_matrix[i][aux] + score_matrix[j][aux]) / 2;
    ...
    while (aux < num_seqs)
    {
        if ((aux != i) && (aux != j))
        {
            if (cluster_equivalent[aux] == cluster_equivalent[i])
            {
                ...
                {
                    if ((aux != n) && (j != n)
                            && (cluster_equivalent[aux] != cluster_equivalent[n]))
                    {
