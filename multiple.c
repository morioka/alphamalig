...
//Descripció: Aquí està la implementació de les funcions per fer laliniament
...
/* alineament_multiple */
/*Funció que portarà a terme laliniament múltiple de les seqüències a
...
void alineament_multiple(float **matriu, int output_format)
{
  ...
  //Vector que conté quines seqüències pertanyen a cada cluster
  ...
  //Vector que conté la posicio de les seqüencies semi-aliniades
  if (mem_ok == 1){
  ...
  //Vector que conté la posicíó de les seqüències al fitxer_cluster_1 o fitxer_cluster_2
  if (mem_ok == 1){
  ...
  //Vector que conté a quin cluster pertany cada seqüencia
  if (mem_ok == 1){
    ...
    if (seq_cluster == NULL){
      ...
      for(i = 0; i < num_seqs; i++) seq_cluster[i] = i;
    }
  }
  
  //Vector que conté les equivalències entre clusters
  if (mem_ok == 1){
    ...
    if (cluster_equivalent == NULL){
      ...
      for(i = 0; i < num_seqs; i++) cluster_equivalent[i] = i;
    }
  }
...
void construir_info_cluster(int num_cluster, float **info, FILE *f)
{
  ...
  while (j < clusters[num_cluster]->long_seqs) {
    ...
    for (k=0;k<numsimb;k++) info[j][k]=0;
    j++;
  }
  //printf("\n2.1.d.2\n");
  //I ara les omplim
...
/*Donats dos indexs de clusters, crea un fitxer per cadascun, i escriu al
  fitxer les seqüències que el formen*/
...
/*Funció que busca les dues seqüències (o clusters) més propers. Retorna el
  resultat com una fila (i) i una columna (j). Aquesta funció només recorre
  la meitat de la matriu, ja que és simètrica.
...
void similitud_clusters(float **matriu, long long_cluster1, long long_cluster2, float **info1, float **info2, long
			*puntuacio,int numseqs1,int numseqs2)
{
  int i = 0, j = 0;
...
float intracluster (float *freq){
  ...
  for (i=0;i<numsimb;i++)
    if (freq[i]>1) sum=sum+freq[i]*(freq[i]-1)*matpenal[i+1][i+1]/2;
  return sum;
}
...
//Amb un vector duns i dosos, i donades les seqüències anteriors, dona el
//cluster resultant
			     ...
			     int len)
{
  ...
  n = 0;
  while (n < num_seqs){
    if ((cluster_equivalent[n] == j) || ((cluster_equivalent[n] == i) && (n != i))) //Només mirem la "j". La "i" no canvia
      {
  ...
  if ((i == 0) && (j == 0)){
    *len = 0;
    ...
    case 'a':
      ...
      (*len)++;
      res1[(*len) - 1] = '1';  //1: hi ha un nucleotid en aquella posició
      res2[(*len) - 1] = '2';  //2: hi ha un gap en aquella posició
      break;
    case 'd':
