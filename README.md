...
※ (修正)アルファベット定義と入力FASTAは hex 形式の入力を受け付ける。出力は `-DHEX_ONLY_MODE` を設定してビルドした際、 hex形式で出力する。
※ (さらに修正)アルファベット定義、入力ファイルに続く3番目のコマンドライン引数として `nonprintable` を指定すると hex形式で出力する。
